...
NUM_CYCLIC_DECAY_CYCLES = 20 # Number of restart cycles for epsilon
NUM_ENVS = 32
...
class QNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        # Fill Lower Triangle
        # Assignment based on user request:
        # Plane 1 (index 0): Player 1 positions (val 1)
        # Plane 2 (index 1): Player 2 positions (val 2)
        # Valid empty spots become 0. Upper triangle remains start_val (-10).
            ...
            for idx in done_indices:
                if pending_obs[idx] is not None:
                    ...
                    if episode_count % EVAL_INTERVAL == 0:
                        ...
                        if eval_wr > SELF_PLAY_UPDATE_THRESHOLD and episodes_since_update >= OPPONENT_UPDATE_MIN_EPISODES:
                            log(f">>> PROMOTING MODEL: (WR: {eval_wr:.2f}) <<<")
                            opponent_model.load_state_dict(current_model.state_dict())
